...
export async function POST() {
  try {
    ...
    try {
      ...
      // Get updated count - use a more robust query
      ...
      // Log the increment
      if (hasNoteColumn) {
        db.prepare('INSERT INTO logs (action, new_value, note) VALUES (?, ?, ?)').run('team_added', result.count, 'Team aanmelding');
      } else {
        db.prepare('INSERT INTO logs (action, new_value) VALUES (?, ?)').run('team_added', result.count);
      }
