...
interface TeamCounterSectionProps {
  onUpdate: () => void;
}

export function TeamCounterSection({ onUpdate }: TeamCounterSectionProps) {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        ...
        setCount(data.count !== undefined && !isNaN(data.count) ? Number(data.count) : 0);
      } catch (error) {
        console.error("Error fetching team count:", error);
        // Set to default value if there's an error
        setCount(0);
      }
    };
  ...
  const handleReset = async () => {
    if (window.confirm("Weet je zeker dat je de team teller wilt resetten naar 0?")) {
      try {
        setIsLoading(true);
        setAnimateCount(true);
    ...
    <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-none shadow-xl overflow-hidden relative group">
      ...
      <CardHeader className="bg-gradient-to-r from-sgpj-blue to-sgpj-blue/90 rounded-t-lg relative z-10">
        ...
        <CardTitle className="text-center text-white text-2xl flex items-center justify-center gap-2">
          ...
          <span>Aantal aanmeldingen teams:</span>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="pt-8 pb-6 relative z-10">
        <div className="flex flex-col items-center">
          <div className="relative mb-8">
            <p className={`text-8xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-sgpj-blue transition-all duration-500 ${animateCount ? 'scale-125' : 'scale-100'}`}>
              {isNaN(count) ? 0 : count}
            </p>
            <div className="absolute -inset-4 bg-sgpj-blue/5 rounded-full blur-xl z-[-1]"></div>
          </div>
